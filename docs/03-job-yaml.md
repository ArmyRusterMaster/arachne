...
proxies:
  ...
  - socks5://5.6.7.8:1080
```
## Чекпоинт (M2): персистентная очередь

`checkpoint` — путь к файлу, куда на каждом шаге сохраняется состояние
(очередь + посещённые URL). При повторном запуске с тем же файлом краулер
продолжает с места остановки, а не начинает заново. Переживает `kill -9`.
...
`next_selector` — CSS-селектор ссылки на следующую страницу. Берётся её
`href`, резолвится относительно текущего URL и ставится в очередь.
Дедупликация по URL предотвращает повторный фетч.
...
## concurrency (задел под Фазу B)

`concurrency` — число параллельных воркеров. По умолчанию `1`
(последовательная обработка). Поле добавлено как задел: полная параллельная
обработка доменов реализуется в Фазе B (см. docs/08-development.md).
